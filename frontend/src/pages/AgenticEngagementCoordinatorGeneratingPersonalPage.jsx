...
export default function AgenticEngagementCoordinatorGeneratingPersonalPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-agentic-engagement-coordinator-generating-personal/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic engagement coordinator generating personalized outreach (visit a newcomer, follow up on absent member)</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-engagement-coordinator-generating-personal/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
